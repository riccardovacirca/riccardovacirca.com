...
apr_array_header_t* aprx_psplit(apr_pool_t *mp, const char *s, const char *sp) {
  ...
  if (str) {
    ...
    while (ptr) {
      *ptr = '\0';
      if (strlen(str) <= 0) {
        tmp = (const char*)aprx_strempty(mp);
        if (tmp) {
...
const char *aprx_datetime_fmt(apr_pool_t *mp, const char *f, int loc) {
  const char *rv = NULL;
  if (mp && f) {
    rv = apr_pstrdup(mp, f);
  }
  if (rv) {
    rv = aprx_pstrrep(mp, aprx_pstrrep(mp, rv, "Y", "%Y"), "y", "%y");
    rv = aprx_pstrrep(mp, aprx_pstrrep(mp, rv, "m", "%m"), "d", "%d");
    rv = aprx_pstrrep(mp, aprx_pstrrep(mp, rv, "H", "%H"), "h", "%I");
    rv = aprx_pstrrep(mp, aprx_pstrrep(mp, rv, "s", "%S"), "i", "%M");
    if (loc) {
      rv = apr_pstrcat(mp, rv, "%z", NULL);
    }
  }
  return rv;
}
...
char* aprx_pdatetime_utc(apr_pool_t *mp, apr_time_t t, const char *f) {
  ...
  if (fm) {
    ...
    apr_status_t st = apr_time_exp_gmt(&tm, t);
    if (st == APR_SUCCESS) {
      st = apr_strftime(tmp, &size, 100, "%Y-%m-%d %H:%M:%S", &tm);
    }
    if (st == APR_SUCCESS) {
      rv = apr_pstrdup(mp, tmp);
    }
...
char* aprx_ppipein(apr_pool_t *mp) {
  char *rv = NULL;
  char buf[APRX_MAX_READ_BUFFER] = {0};
  apr_size_t l;
  apr_file_t *fd;
  apr_size_t bytes = APRX_MAX_READ_BUFFER - 1;
